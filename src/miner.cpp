...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("COSOMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("COSOMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("COSOMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("coso-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("COSOMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running COSOMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadCOSOMiner() exception");
    } catch (...) {
        LogPrintf("ThreadCOSOMiner() exception");
    }

    LogPrintf("ThreadCOSOMiner exiting\n");
}
