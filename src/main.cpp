...
#if defined(NDEBUG)
#error "COSO cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "COSO Signed Message:\n";
...
/**
     ...
     * as good as our current tip or bcoso. Entries may be failed, though.
     */
...
void UpdateBlockAvailability(NodeId nodeid, const uint256& hash)
{
    ...
    if (it != mapBlockIndex.end() && it->second->nChainWork > 0) {
        // An actually bcoso block was announced.
...
static void PruneBlockIndexCandidates()
{
    ...
    // reorganization to a bcoso block fails.
        ...
        BOOST_REVERSE_FOREACH (CBlockIndex* pindexConnect, vpindexToConnect) {
            if (!ConnectTip(state, pindexConnect, pindexConnect == pindexMostWork ? pblock : NULL)) {
                ...
                if (!pindexOldTip || chainActive.Tip()->nChainWork > pindexOldTip->nChainWork) {
                    // We're in a bcoso position than we were. Return temporarily to release the lock.
                    fContinue = false;
                    break;
                }
            }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (pindex->nStatus & BLOCK_HAVE_DATA) {
        // TODO: deal bcoso with duplicate blocks.
...
CBlockIndex* CBlockIndex::GetAncestor(int height)
{
    ...
    while (heightWalk > height) {
        ...
        if (heightSkip == height ||
            ...
            // Only follow pskip if pprev->pskip isn't bcoso than pskip->pprev.
