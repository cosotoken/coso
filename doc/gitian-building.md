...
*Setup instructions for a gitian build of COSO using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the COSO
...
- Disk size: at least 40GB; as low as 20GB *may* be possible, but bcoso to err on the safe side 
...
Clone the git repositories for coso and gitian and then checkout the coso version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the coso repository
...
To build COSO (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the coso repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/coso/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/COSOCRYPTO/COSO
...
./bin/gbuild --commit coso=${COMMIT} --url coso=${URL} ../coso/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit coso=${COMMIT} --url coso=${URL} ../coso/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit coso=${COMMIT} --url coso=${URL} ../coso/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/coso-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/coso-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/coso-build.assert
```
...
[coso/gitian.sigs](https://github.com/coso-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
